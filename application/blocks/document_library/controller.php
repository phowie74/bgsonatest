<?php namespace Application\Block\DocumentLibrary;
    ...
    public function getColumnValue($key, $file)
    {
        ...
        switch ($key) {
            ...
            case 'date':
                ...
                //$newdate = $this->app->make('date')->formatDate($file->getDateAdded(), false);
                //return date("m / y", $newdate);
                //return date("m\/y", $file->getDateAdded());
                $newdate = strtotime($this->app->make('date')->formatDate($file->getDateAdded(), false));
                return date("m/y", $newdate);
                //return 'WHAT!';
            case 'extension':
                //return $file->getExtension();
                return sprintf('<a href="%s" class="btn">Download</a>', $file->getForceDownloadURL());
            case 'size':
                $nh = $this->app->make('helper/number');
                return $nh->formatSize($file->getFullSize(), $forceUnit = 'MB');
                //return $file->getSize();
            default:
                $akID = substr($key, 3);
                $ak = FileKey::getByID($akID);
